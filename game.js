...
function spawnCloud() { clouds.push({ x: canvas.width, y: Math.random() * (canvas.height / 3), width: Math.random() * 100 + 80, height: Math.random() * 40 + 30, speedX: -(Math.random() * 0.5 + 0.2), color: `rgba(255, 255, 255, ${Math.random() * 0.4 + 0.5})` }); }
function spawnBird() { birds.push({ x: canvas.width, y: Math.random() * (canvas.height / 2 - 50) + 20, width: 20, height: 10, speedX: -(Math.random() * 2 + 1), color: '#5C4033' }); }
function spawnCoin() { coins.push({ x: canvas.width, y: Math.random() * (platforms[0].y - 250) + 150, width: 15, height: 15, speedX: -baseSpeed * 0.8, color: '#f1c40f' }); }
...
function update() {
    ...
    if (newElapsedTime !== elapsedTime) elapsedTime = newElapsedTime;
    ...
    enemyTimer++; cloudTimer++; birdTimer++; coinTimer++;
    ...
    for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        if (enemy.x + enemy.width < 0 || enemy.x > canvas.width || enemy.y > canvas.height) enemies.splice(i, 1);
    }
